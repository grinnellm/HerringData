...
load_bio <- function(
  ...
  db_connection <- dbConnect(
    odbc::odbc(),
    driver = db_info$herring_conn$driver,
  ...
  sql_samples <- paste(
    ...
    "FROM", paste(
      db_info$bio_loc$schema, db_info$bio_loc$tables$samples,
      sep = "."
    )
  )
  ...
  sql_fish <- paste(
    ...
    "FROM", paste(
      db_info$bio_loc$schema, db_info$bio_loc$tables$fish,
      sep = "."
    )
  )
  ...
  fish <- fish_dat %>%
    ...
    mutate(Year = season_to_year(season)) %>%
    select(
...
load_catch <- function(
  ...
  sql_tab <- paste(
    ...
    "FROM", paste(
      db_info$catch_loc$schema, db_info$catch_loc$tables$tab_catch,
      sep = "."
    )
  )
  ...
  sql_hail <- paste(
    ...
    "FROM", paste(
      db_info$catch_loc$schema, db_info$catch_loc$tables$hail_catch,
      sep = "."
    )
  )
  ...
  sql_sok <- paste(
    ...
    "FROM", paste(
      db_info$catch_loc$schema, db_info$catch_loc$tables$sok_catch,
      sep = "."
    )
  )
  ...
  res <- all_catch %>%
    ...
    mutate(
      Period = 0,
      Period = ifelse(
        Source == "Tab" & DisposalCode %in% c(1, 3, 4, 5, 6),
        1, Period
      ),
      ...
      Period = ifelse(
        Source == "Tab" & DisposalCode %in% c(7, 8) &
          GearCode %in% c(29),
        2, Period
      ),
      # If include test catch, disposal %in% c(7, 8), else disposal == 8
