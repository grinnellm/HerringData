---
title: Introduction to SISCAH data
...
csl: ../inst/csas.csl
vignette: >
  %\VignetteIndexEntry{Introduction to SISCAH data}
...
region <- "HG"
substock <- "Section"
yrs_plot <- c(pars$years$assess, 2025)
```

This introduction shows an example workflow in the
`r region` stock assessment region (SAR) using
`r substock` for sub-stock structure from
`r min(yrs_plot)` to `r max(yrs_plot)`.
...
## Catch data {#sec:load-catch-data}
...
```{r catch}
  ...
  left_join(y = codes_period, by = "Gear") %>%
  select(Stock, Area, Period, Year, Value) %>%
  mutate(Period = factor(Period, levels = codes_period$Period))
```

Catch in thousands of tonnes by stock, period, and year,
except SOK catch which is in thousands of pounds.
Here we convert SOK catch to the same scale as the other fisheries.
Note that table layout is similar to the biological data tables.

```{r catch-catch}
catch <- catch %>%
  mutate(
    Value = ifelse(Period == "SOK", Value * conv_factors$lb2kg / 1000, Value)
  )
catch
```
...
```{r num-age-plot, fig.width = 8, fig.height = 5, fig.cap = num_age_cap, warning = FALSE}
num_age_cap <- paste(
...
```{r prop-age-plot, fig.width = 8, fig.height = 6, fig.cap = prop_age_cap}
prop_age_cap <- paste(
...
```{r wt-age-plot, fig.width = 8, fig.height = 6, fig.cap = wt_age_cap}
wt_age_cap <- paste(
...
```{r wt-age-sn-plot, fig.width = 8, fig.height = 6, fig.cap = wt_age_sn_cap}
wt_age_sn_cap <- paste(
  region, "weight-at-age in kilograms (kg) from",
...
Note that we converted SOK catch to the same scale as the other fisheries
(see Section \@ref(sec:load-catch-data)).

```{r catch-plot, fig.width = 8, fig.height = 6, fig.cap = catch_cap}
catch_cap <- paste(
  region, "catch in thousands of tonnes (t) from",
  paste(yrs_plot, collapse = " to "), "by stock and period."
)
catch_plot <- ggplot(
  data = catch, mapping = aes(x = Year, y = Value, fill = Period)
) +
  geom_col() +
...
SOK catch is quite small and hard to see relative to the other periods
so zoom in to see SOK.

```{r catch-zoom-plot, fig.width = 8, fig.height = 6, fig.cap = catch_zoom_cap}
yrs_plot_zoom <- pars$years$dive:yrs_plot[2]
catch_zoom <- catch %>% 
  filter(Year %in% yrs_plot_zoom)
catch_zoom_cap <- paste(
  region, "catch in thousands of tonnes (t) from",
  paste(range(yrs_plot_zoom), collapse = " to "), "by stock and period."
)
catch_zoom_plot <- ggplot(
  data = catch_zoom, mapping = aes(x = Year, y = Value, fill = Period)
) +
  geom_col() +
  scale_fill_viridis_d() +
  labs(y = "Catch (1,000 t)") +
  expand_limits(x = yrs_plot_zoom) +
...
```{r spawn-plot, fig.width = 8, fig.height = 6, fig.cap = spawn_cap}
spawn_cap <- paste(
